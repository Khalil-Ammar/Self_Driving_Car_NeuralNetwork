...
def backpropagation(x, t, W, B):
    ...
    delta_out = derivative_activation(z_s[-1]) * (t - a_s[-1])  ## scalar values
    
    delta = [None]*len(W)
    delta[-1] = delta_out
    d_b[-1] = -delta_out
    d_w[-1] = np.dot(delta[-1], a_s[-2].T) ## (i=1): a_s[1] = 10x1, delta[1] = 1x1;
    
    for i in range(len(W)-2, -1, -1):
        delta_i = np.dot(W[i+1].T, delta[i+1]) * derivative_activation(z_s[i+1])  ## eg i = 0 : W[1]' = 10x1, delta[1] = 1x1, z_s[1] = 10x1 ==> result = 10x1
        delta[i] = delta_i
        d_b[i] = -delta[i]
        d_w[i] = np.dot(delta[i], a_s[i].T)  ##(i=0): a_s[0] = 784x1, delta[0] = 10x1
